...
os.environ['TF_ENABLE_ONEDNN_OPTS'] = '0' # Отключение предупреждения
os.environ['XFORMERS_MORE_DETAILS'] = '1' # Выводим недостающие модули xFormers
...
TRAIN_DATA_DIR                  = "dataset"                               # Папка с изображениями для обучающего набора данных
VALIDATION_RATIO                = 0.05                                    # Сколько % данных изъять для валидационного набора
RECREATE_CACHE                  = False                                   # Пересоздать все кэши датасета. Не забудьте сменить сид
...
# Настройки обучения
LEARNING_RATE                   = 1e-5                                    # Начальная скорость обучения (после возможного периода разогрева), которая будет использоваться   
REPEATS                         = 1                                       # Количество повторов датасета в рамках одной эпохи
...
GRAD_ACCUMULATION_STEPS         = 4                                       # Количество шагов обновления, которые необходимо накопить перед выполнением обратного прохода и обновления параметров
...
LR_WARMUP_STEPS                 = 1500                                    # Количество шагов разогрева в планировщике скорости обучения
COSINE_RESTARTS                 = 4                                       # Для планировщика LR_SCHEDULER cosine_with_restarts. Задает количество рестартов
...
# Параметры LoRA
LORA_RANK                       = 256                                     # Размерность матриц обновления LoRA
LORA_ALPHA                      = 256                                     # Масштабный коэффициент, управляющий влиянием адаптационных весов LoRA
...
CAPTION_DROPOUT                 = 0.25                                    # Вероятность удаления подписи и обновления безусловного пространства
MASK_DROPOUT                    = 0.25                                    # Вероятность замены маски на нули
...
SAVE_NAME                       = "flux_fill"                             # Префикс имени для сохранения контрольных точек
SAVE_MODEL_EPOCHS               = 0                                       # Сохранять модель каждые N эпох
SAVE_MODEL_STEPS                = 1000                                    # Сохранять модель каждые N шагов
...
def create_empty_embedding(tokenizers,text_encoders,cache_path="cache/empty_embedding.npflux",recreate=False):
    ...
    # Пересоздание кэша
    if recreate and os.path.exists(cache_path):
